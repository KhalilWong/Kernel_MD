...
__global__ void Boundary_XY(Gas_Molecule *Ar, Wall_Molecule *Pt, Parameters_MD *Pars_MD) {
	...
	if (tid < Pt_N) {
		//X
		if (Pt[tid].x < (*Pars_MD).BoxXLow) {
			Pt[tid].x += (*Pars_MD).BoxXHigh - (*Pars_MD).BoxXLow;
			Pt[tid].Bx += (*Pars_MD).BoxXHigh - (*Pars_MD).BoxXLow;
		}
		else if (Pt[tid].x >= (*Pars_MD).BoxXHigh) {
			Pt[tid].x -= (*Pars_MD).BoxXHigh - (*Pars_MD).BoxXLow;
			Pt[tid].Bx -= (*Pars_MD).BoxXHigh - (*Pars_MD).BoxXLow;
		}
		//Y
		if (Pt[tid].y < (*Pars_MD).BoxYLow) {
			Pt[tid].y += (*Pars_MD).BoxYHigh - (*Pars_MD).BoxYLow;
			Pt[tid].By += (*Pars_MD).BoxYHigh - (*Pars_MD).BoxYLow;
		}
		else if (Pt[tid].y >= (*Pars_MD).BoxYHigh) {
			Pt[tid].y -= (*Pars_MD).BoxYHigh - (*Pars_MD).BoxYLow;
			Pt[tid].By -= (*Pars_MD).BoxYHigh - (*Pars_MD).BoxYLow;
		}
	}
...
__global__ void RescaleT2(Wall_Molecule *Pt, Parameters_Wall *Pars_Pt) {
	...
	//
	if (tid<Pt_N) {
		Pt[tid].vx -= (*Pars_Pt).ArgVelX;
		Pt[tid].vy -= (*Pars_Pt).ArgVelY;
		Pt[tid].vz -= (*Pars_Pt).ArgVelZ;
	}
}
...
__global__ void RescaleT4(Wall_Molecule *Pt, Parameters_Wall *Pars_Pt) {
	...
	//
	if (tid<Pt_N) {
		Pt[tid].vx *= sqrt((*Pars_Pt).T / (*Pars_Pt).CurrentT);
		Pt[tid].vy *= sqrt((*Pars_Pt).T / (*Pars_Pt).CurrentT);
		Pt[tid].vz *= sqrt((*Pars_Pt).T / (*Pars_Pt).CurrentT);
	}
}
	...
	if (tid<Pt_N) {
		...
		for (i = 0; i < Pt_N + Ar_N; i++) {
			if (i<Pt_N) {
				...
				Pairx = Pt[tid].x - Ar[0].x;
				Pairy = Pt[tid].y - Ar[0].y;
				Pairz = Pt[tid].z - Ar[0].z;
			}
			//
		...
		Spring_Disx = Pt[tid].x - Pt[tid].Bx;
		Spring_Fx = -(*Pars_MD).SprK * Spring_Disx;
		Pt_Fx = Atom_Fx + Spring_Fx;
		Pt[tid].ax = Pt_Fx / (*Pars_Pt).Mass;
		Spring_Disy = Pt[tid].y - Pt[tid].By;
		Spring_Fy = -(*Pars_MD).SprK * Spring_Disy;
		Pt_Fy = Atom_Fy + Spring_Fy;
		Pt[tid].ay = Pt_Fy / (*Pars_Pt).Mass;
		Spring_Disz = Pt[tid].z - Pt[tid].Bz;
		Spring_Fz = -(*Pars_MD).SprK * Spring_Disz;
		Pt_Fz = Atom_Fz + Spring_Fz;
		Pt[tid].az = Pt_Fz / (*Pars_Pt).Mass;
	}
	//
	if (tid == Pt_N) {
		//
...
__global__ void Verlet_Pos(Gas_Molecule *Ar, Wall_Molecule *Pt, Parameters_MD *Pars_MD) {
	...
	//
	if (tid<Pt_N) {
		Pt[tid].x += Pt[tid].vx * (*Pars_MD).dt + 0.5 * Pt[tid].ax * (*Pars_MD).dt * (*Pars_MD).dt;
		Pt[tid].y += Pt[tid].vy * (*Pars_MD).dt + 0.5 * Pt[tid].ay * (*Pars_MD).dt * (*Pars_MD).dt;
		Pt[tid].z += Pt[tid].vz * (*Pars_MD).dt + 0.5 * Pt[tid].az * (*Pars_MD).dt * (*Pars_MD).dt;
	}
	if (tid == Pt_N) {
		Ar[0].x += Ar[0].vx * (*Pars_MD).dt + 0.5 * Ar[0].ax * (*Pars_MD).dt * (*Pars_MD).dt;
		Ar[0].y += Ar[0].vy * (*Pars_MD).dt + 0.5 * Ar[0].ay * (*Pars_MD).dt * (*Pars_MD).dt;
		Ar[0].z += Ar[0].vz * (*Pars_MD).dt + 0.5 * Ar[0].az * (*Pars_MD).dt * (*Pars_MD).dt;
	}
}
...
__global__ void Verlet_Vel(Gas_Molecule *Ar, Wall_Molecule *Pt, Parameters_MD *Pars_MD) {
	...
	//
	if (tid<Pt_N) {
		Pt[tid].vx += 0.5 * (Pt[tid].atx + Pt[tid].ax) * (*Pars_MD).dt;
		Pt[tid].vy += 0.5 * (Pt[tid].aty + Pt[tid].ay) * (*Pars_MD).dt;
		Pt[tid].vz += 0.5 * (Pt[tid].atz + Pt[tid].az) * (*Pars_MD).dt;
	}
	if (tid == Pt_N) {
		Ar[0].vx += 0.5 * (Ar[0].atx + Ar[0].ax) * (*Pars_MD).dt;
		Ar[0].vy += 0.5 * (Ar[0].aty + Ar[0].ay) * (*Pars_MD).dt;
		Ar[0].vz += 0.5 * (Ar[0].atz + Ar[0].az) * (*Pars_MD).dt;
	}
}
...
__global__ void Last_Acceleration(Gas_Molecule *Ar, Wall_Molecule *Pt) {
	...
	//
	if (tid<Pt_N) {
		Pt[tid].atx = Pt[tid].ax;
		Pt[tid].aty = Pt[tid].ay;
		Pt[tid].atz = Pt[tid].az;
	}
	if (tid == Pt_N) {
		Ar[0].atx = Ar[0].ax;
		Ar[0].aty = Ar[0].ay;
