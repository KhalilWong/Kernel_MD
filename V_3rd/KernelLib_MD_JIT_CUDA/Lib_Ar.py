...
def Dis_Vel(Name,Atoms,N,mpVelocity,Axis,XL,XH,YL,YH):
    ...
    #
    Temp=[int(round(Atoms[i].vx*AccN)) for i in range(N)]
    Max=max(Temp)
    Min=min(Temp)
    Delta=Min
    X1=[]
    ...
    #
    Temp=[int(round(Atoms[i].vy*AccN)) for i in range(N)]
    Max=max(Temp)
    Min=min(Temp)
    Delta=Min
    X2=[]
    ...
    #
    Temp=[int(round(Atoms[i].vz*AccN)) for i in range(N)]
    Max=max(Temp)
    Min=min(Temp)
    Delta=Min
    X3=[]
    ...
    #
    #fig4=mpl.figure()
    #lab='Z-Velocity Distribution of '+str(N)+' '+Name+' Atoms'
    #mpl.plot(X3,Y3,'ro',markersize=4,label=lab)
    #mpl.plot(MX3,MY3,'b',markersize=1,label='Maxwellian')
    #mpl.legend(loc='upper right')
    #mpl.xlabel('$vz$')
    #mpl.ylabel('$D$')
    #mpl.axis([-3.0,0.0,0.0,1.2])
    #mpl.savefig(Name+'_D_vz.png',dpi=600)
    #
    #mpl.show()
    mpl.close()
...
if __name__ == '__main__':
    ...
    Ar_Pars.Set_T(MD_Pars.kB,350.0,2)
    Ar_Pars.Non_Dim(Dim)
    #
    Lib_Ar=[]
    count=0
    ...
    start=time.time()
    while(MD_Pars.State):
        TA.Time_Advancement[BD,512](d_Type,d_Elasticity,d_Pos,d_Vel,d_Acc,Box,ZLow,LJ_E,LJ_S,Mass,Pt_argVel,Pt_V2,Pt_T,T,Pt_N,MD_Pars.dt,MD_Pars.cutoff,MD_Pars.Spr_K,Dim.Velocity,Dim.Mass,MD_Pars.kB,Pt_N,MD_Pars.TimeStep)
