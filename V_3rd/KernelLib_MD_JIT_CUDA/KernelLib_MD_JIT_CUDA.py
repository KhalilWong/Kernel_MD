...
class Parameters():
    ...
    def __init__(self):
        ...
        self.Box[0,0]=0
        self.Box[0,1]=self.Pt_I*self.fcc_lattice
        self.Box[0,2]=self.Box[0,1]-self.Box[0,0]
        self.Box[1,0]=0
        self.Box[1,1]=self.Pt_J*self.fcc_lattice
        self.Box[1,2]=self.Box[1,1]-self.Box[1,0]
        self.Box[2,0]=-(self.Pt_K-0.5)*self.fcc_lattice
        self.Box[2,1]=self.d
        self.Box[2,2]=self.Box[2,1]-self.Box[2,0]
        print('计算区域X: ',self.Box[0,0],', ',self.Box[0,1])
        print('计算区域Y: ',self.Box[1,0],', ',self.Box[1,1])
        print('计算区域Z: ',self.Box[2,0],', ',self.Box[2,1])
        ...
        self.Lib_N=1000
        self.Blocks=1000
        self.ThreadsPerBlock=self.All_N
        print('*******Parameters Setted!*******')


################################################################################
def InitializationLib(Pars):
    ArLib_Pos=np.zeros((Pars.Lib_N,3))
    ArLib_Vel=np.zeros((Pars.Lib_N,3))
    d_ArLib_Pos=cuda.to_device(ArLib_Pos)
    d_ArLib_Vel=cuda.to_device(ArLib_Vel)
    seedt=int(time.time())
    rng_states=create_xoroshiro128p_states(Pars.Lib_N,str(seedt))
    InitializationLib_Kernel[int(Pars.Lib_N+1023/1024),1024](d_ArLib_Pos,d_ArLib_Vel,Pars.Lib_N,rng_states,Pars.Box,Pars.mp_V,Pars.pi)
    ArLib_Pos=d_ArLib_Pos.copy_to_host()
    ArLib_Vel=d_ArLib_Vel.copy_to_host()
    print('创建了包含 ',Pars.Lib_N,' 个Ar分子的库')

    return(ArLib_Pos,ArLib_Vel,seedt)


################################################################################
def InitializationSample(ArLib_Pos,ArLib_Vel,Pars,SID,seedt):
    timestep=0
    #位置，速度初始化
    All_Pos=np.zeros((Pars.All_N,3))
    All_Vel=np.zeros((Pars.All_N,3))
    #Ar
    All_Pos[Pars.Pt_N,:]=ArLib_Pos[SID,:]
    All_Vel[Pars.Pt_N,:]=ArLib_Vel[SID,:]
    #Pt
    d_All_Pos=cuda.to_device(All_Pos)
    d_All_Vel=cuda.to_device(All_Vel)
    seedt+=SID+1
    rng_states=create_xoroshiro128p_states(Pars.All_N,str(seedt))
    InitializationPt_Kernel[1,Pars.Pt_N](d_All_Pos,d_All_Vel,Pars.All_type,Pars.Pt_N,Pars.Pt_I,Pars.Pt_J,Pars.Pt_K,Pars.fcc_lattice,rng_states,Pars.mp_V)
    #首次位置周期
...
################################################################################
@cuda.jit
def InitializationLib_Kernel(ArLib_Pos,ArLib_Vel,Lib_N,rng_states,Box,mp_V,pi):
    t_pos=cuda.grid(1)
    if(t_pos<Lib_N):
        Rx=xoroshiro128p_uniform_float64(rng_states, t_pos)
        Ry=xoroshiro128p_uniform_float64(rng_states, t_pos)
        ArLib_Pos[t_pos,0]=Box[0,0]+Box[0,2]*Rx
        ArLib_Pos[t_pos,1]=Box[1,0]+Box[1,2]*Ry
        ArLib_Pos[t_pos,2]=Box[2,1]
        R1=0
        while R1==0:
        ...
        ArLib_Vel[t_pos,0]=mp_V[0]*math.sqrt(-math.log(R1))*math.cos(2*pi*R2)#Maxwell分布
        R1=0
        while R1==0:
            R1=xoroshiro128p_uniform_float64(rng_states, t_pos)
        R2=0
        while R2==0:
            R2=xoroshiro128p_uniform_float64(rng_states, t_pos)
        ArLib_Vel[t_pos,1]=mp_V[0]*math.sqrt(-math.log(R1))*math.sin(2*pi*R2)
        R1=0
        while R1==0:
            R1=xoroshiro128p_uniform_float64(rng_states, t_pos)
        R2=0
        while R2==0:
            R2=xoroshiro128p_uniform_float64(rng_states, t_pos)
        ArLib_Vel[t_pos,2]=-mp_V[0]*math.sqrt(-math.log(R1))
        cuda.syncthreads()


################################################################################
@cuda.jit
def InitializationPt_Kernel(All_Pos,All_Vel,All_type,Pt_N,Pt_I,Pt_J,Pt_K,fcc_lattice,rng_states,mp_V):
    tx=cuda.threadIdx.x
    bx=cuda.blockIdx.x
    tpb=cuda.blockDim.x
    t_pos=tx+tpb*bx
    if(t_pos<Pt_N):
        ...
        for axis in range(3):
            ...
            All_Vel[t_pos,axis]=mp_V[1]/math.sqrt(3.0)*R#高斯分布，平均值为0，方差=方均根速度用来控温
        All_type[t_pos]=1.0
        cuda.syncthreads()
...
Pars=Parameters()
(ArLib_Pos,ArLib_Vel,seedt)=InitializationLib(Pars)
while()
(All_Pos,All_Vel,All_Acc,timestep)=InitializationSample(ArLib_Pos,ArLib_Vel,Pars,SID,seedt)
Ar_Z=[]
Ar_t=[]
