...
def ReadFile(FileName):
    ...
    with open(FileName,'r') as In:
        ...
        Adsorbed=[]
        for pdata in InData:
            (id,tt,cn,x,y,z,vx,vy,vz,fx,fy,fz,fvx,fvy,fvz,ad)=pdata.split('\t',15)
            ID.append(int(id))
            Tt.append(float(tt))
            CN.append(int(cn))
            X.append(float(x))
    ...
    #
    return(ID,Tt,CN,X,Y,Z,VX,VY,VZ,FX,FY,FZ,FVX,FVY,FVZ,Adsorbed)
################################################################################
...
def main():
    ...
    (ID,Tt,CN,X,Y,Z,VX,VY,VZ,FX,FY,FZ,FVX,FVY,FVZ,Adsorbed)=ReadFile(FileName)
    #
    GasT=300.0
    WallT=300.0
