...
def SaveOldGetNew(i,j,Lib_N,Block_N,Sid,Pos,Vel,Acc,Ar_X,Ar_Y,Ar_Z,Ar_VX,Ar_VY,Ar_VZ,Ar_AX,Ar_AY,Ar_AZ,Pt_X,Pt_Y,Pt_Z,Pt_BX,Pt_BY,Pt_BZ,Pt_VX,Pt_VY,Pt_VZ,Pt_AX,Pt_AY,Pt_AZ,Pt_N,Ar_N):
    #
    while(j<Block_N):
        if(Sid[j,2]>0):
            #
            ...
            with open('Incident_Reflection.data','a') as IR:
                if(Sid[j,2]==2):
                    ...
                    Adsorbed='False'
                print('%d\t%d\t%d\t%f\t%f\t%f\t%f\t%f\t%f\t%f\t%f\t%f\t%f\t%f\t%f\t%s'%(int(Sid[j,0]),int(Sid[j,1]),int(Sid[j,3]),Ar_X[int(Sid[j,0])],Ar_Y[int(Sid[j,0])],Ar_Z[int(Sid[j,0])],Ar_VX[int(Sid[j,0])],Ar_VY[int(Sid[j,0])],Ar_VZ[int(Sid[j,0])],Final_Ar_X,Final_Ar_Y,Final_Ar_Z,Final_Ar_VX,Final_Ar_VY,Final_Ar_VZ,Adsorbed),file=IR)
            #
            if(i<Lib_N):
                Sid[j,0]=i
                Sid[j,1]=0
...
def main():
    ...
    Sid=np.zeros((Block_N,4))                                                   #全局ID；已运行时间步；状态（-1：无新的算例，线程关闭；0：未完成；1：散射；2：吸附）；与壁面碰撞次数
...
#        print(str(kern.inspect_llvm()),file=out)
    ...
    with open('Incident_Reflection.data','w') as IR:
        print('ID\tTt\tCN\tIX\tIY\tIZ\tIVX\tIVY\tIVZ\tFX\tFY\tFZ\tFVX\tFVY\tFVZ\tState',file=IR)
    TotalLoop=0
    ...
    while(State):
        ...
        if(CompleteS[0]!=OldCompleteS[0]):
            ...
            for i in range(int(OldCompleteS[0])+1,int(CompleteS[0])+1):
                ...
                #
                SaveOldGetNew(i,j,Lib_N,Block_N,Sid,Pos,Vel,Acc,Ar_X,Ar_Y,Ar_Z,Ar_VX,Ar_VY,Ar_VZ,Ar_AX,Ar_AY,Ar_AZ,Pt_X,Pt_Y,Pt_Z,Pt_BX,Pt_BY,Pt_BZ,Pt_VX,Pt_VY,Pt_VZ,Pt_AX,Pt_AY,Pt_AZ,Pt_N,Ar_N)
                if(i==CompleteS[0]):
