...
public:
	...
	void Initialization_Kernel(int All_type[], double All_Pos[][3], double All_Vel[][3]);
...
void Parameters::Initialization(int All_type[], double All_Pos[][3], double All_Vel[][3], double All_Acc[][3]) {
	double *Pt_argVelx, *Pt_argVely, *Pt_argVelz, *Pt_V2, *d_Box, *d_Pt_ePos, d_nd_Velocity, *d_Mass, d_nd_Mass, d_kB, *d_T, *d_LJ_E, *d_LJ_S, d_cutoff, d_spr_k;
	int i, *ZLow;
	double (*d_All_Pos)[3], (*d_All_Vel)[3], (*d_All_Acc)[3], (*d_Box)[2], (*d_Pt_ePos)[3];
	__global__ void Pos_period(double All_Pos[][3], double Box[3][2], double Pt_ePos[][3], int *ZLow);
	...
	__global__ void Acceleration_period(double All_Pos[][3], double All_Acc[][3], double LJ_E[3], double LJ_S[3], double Box[3][2], double cutoff, double Pt_ePos[][3], double spr_k, double Mass[2]);
	...
	//位置，速度初始化
	Initialization_Kernel(All_type, All_Pos, All_Vel);
	//分配内存，初始化
	...
	cudaMallocManaged(&ZLow, sizeof(int));
	cudaMallocManaged(&Pt_argVelx, sizeof(double));
	cudaMallocManaged(&Pt_argVely, sizeof(double));
	cudaMallocManaged(&Pt_argVelz, sizeof(double));
	cudaMallocManaged(&Pt_V2, sizeof(double));
	...
	*Pt_V2 = 0.0;
	//首次位置周期
	Pos_period << <1, Pt_N + Ar_N >> >(d_All_Pos, d_Box, d_Pt_ePos, ZLow);
	cudaDeviceSynchronize();
	Box[2][0] = (*ZLow) / 1000.0;
	cudaMemcpy(d_Box, Box, sizeof(Box), cudaMemcpyHostToDevice);
	//首次控温
	rescale_T << <1, Pt_N >> >(d_All_Vel, Pt_argVelx, Pt_argVely, Pt_argVelz, Pt_V2, d_nd_Velocity, d_Mass, d_nd_Mass, d_kB, d_T);
	cudaDeviceSynchronize();
	//首次加速度周期
	...
	cudaMemcpy(Pt_ePos, d_Pt_ePos, sizeof(Pt_ePos), cudaMemcpyDeviceToHost);
	cudaFree(d_All_Pos);
	cudaFree(d_All_Vel);
	cudaFree(d_All_Acc);
	cudaFree(d_Box);
...
/******************************************************************************/
void Parameters::Initialization_Kernel(int All_type[], double All_Pos[][3], double All_Vel[][3]) {
			...
			for (k = 0; k < 2 * Pt_K; k++) {
				if (i / 2. + j / 2. + k / 2. == int(i / 2. + j / 2. + k / 2.)) {
					...
					for (axis = 0; axis < 3; axis++) {
						R1 = random();
						R2 = random();
						All_Vel[count][axis] = mp_V[1] / sqrt(3)*sqrt(-2 * log(R1))*cos(2 * PI*R2);//��˹�ֲ���ƽ��ֵΪ0������=�������ٶ���������
					}
					count += 1;
				}
			}
	...
	R1 = random();
	R2 = random();
	All_Vel[count][0] = mp_V[0] * sqrt(-log(R1))*cos(2 * PI*R2);//Maxwell�ֲ�
	R1 = random();
	R2 = random();
...
__global__ void Pos_period(double All_Pos[][3], double Box[3][2], double Pt_ePos[][3], int *ZLow) {
	...
	if (tid<Pt_N + Ar_N) {
		...
		//Z方向下边界更新
		atomicMin(ZLow, int(All_Pos[tid][2]*1000));
	}
}
...
__global__ void rescale_T(double All_Vel[][3], double *Pt_argVelx, double *Pt_argVely, double *Pt_argVelz, double *Pt_V2, double nd_Velocity, double Mass[2], double nd_Mass, double kB, double T[2]) {
	...
	if (tid<Pt_N) {
		atomicAdd(Pt_argVelx, (All_Vel[tid][0]) / Pt_N);
		atomicAdd(Pt_argVely, (All_Vel[tid][1]) / Pt_N);
		atomicAdd(Pt_argVelz, (All_Vel[tid][2]) / Pt_N);
		__syncthreads();
		//只需要热运动速度
		All_Vel[tid][0] -= *Pt_argVelx;
		All_Vel[tid][1] -= *Pt_argVely;
		All_Vel[tid][2] -= *Pt_argVelz;
		atomicAdd(Pt_V2, All_Vel[tid][0] * All_Vel[tid][0] + All_Vel[tid][1] * All_Vel[tid][1] + All_Vel[tid][2] * All_Vel[tid][2]);
		__syncthreads();
		Pt_T = (*Pt_V2) * nd_Velocity* nd_Velocity * Mass[1] * nd_Mass / (3 * Pt_N*kB);
...
int main() {
	...
	double All_Pos[Pt_N + Ar_N][3], All_Vel[Pt_N + Ar_N][3], All_Acc[Pt_N + Ar_N][3];
	double (*d_All_Pos)[3], (*d_All_Vel)[3], (*d_All_Acc)[3], (*d_All_Acc_temp)[3], *Pt_argVelx, *Pt_argVely, *Pt_argVelz, *Pt_V2;
	double *d_dt, *d_Box, *d_Pt_ePos, d_nd_Velocity, *d_Mass, d_nd_Mass, d_kB, *d_T, *d_LJ_E, *d_LJ_S, d_cutoff, d_spr_k;
	int timestep, *ZLow;
	__global__ void Verlet_Pos(double All_Pos[][3], double All_Vel[][3], double All_Acc[][3], double dt);
	__global__ void Pos_period(double All_Pos[][3], double Box[3][2], double Pt_ePos[][3], int *temp);
	__global__ void Acceleration_period(double All_Pos[][3], double All_Acc[][3], double LJ_E[3], double LJ_S[3], double Box[3][2], double cutoff, double Pt_ePos[][3], double spr_k, double Mass[2]);
	__global__ void Verlet_Vel(double All_Vel[][3], double All_Acc_temp[][3], double All_Acc[][3], double dt);
	__global__ void rescale_T(double All_Vel[][3], double *Pt_argVelx, double *Pt_argVely, double *Pt_argVelz, double *Pt_V2, double nd_Velocity, double Mass[2], double nd_Mass, double kB, double T[2]);
	...
	cudaMemcpy(d_nd_Velocity, Pars.nd_Velocity, sizeof(double), cudaMemcpyHostToDevice);
	cudaMemcpy(d_Mass, Pars.Mass, sizeof(Pars.Mass), cudaMemcpyHostToDevice);
	cudaMemcpy(d_nd_Mass, Pars.nd_Mass, sizeof(double), cudaMemcpyHostToDevice);
	cudaMemcpy(d_kB, Pars.kB, sizeof(double),cudaMemcpyHostToDevice);
